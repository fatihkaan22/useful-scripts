...
# script that shows duration of video files in directory
...
get_duration(){
    # /usr/bin/ffprobe "$1" 2>&1 | grep Duration | awk -F[:,] '{print int($2*3600+$3*60+$4)}'
		durationMs=$(find "$1" -type f -exec mediainfo --Inform="General;%Duration%" "{}" \; 2>/dev/null)
...
execute_it_now(){
    ...
    echo $mysmallsep
    j=0
    for i in "$1"/{*.mp4,*.mkv,*.avi,*.flv,*.webm} ## Put the existing video file extension you have
    do
